...
const CONFIG = {
  ...
  version: process.env.ENGINE_VERSION
    ? parseInt(process.env.ENGINE_VERSION)
    : 14,
  requestDelay: 300,
...
class TestHarness {
  ...
  async initialize() {
    ...
    console.log(
      `   Program ID: ${colorize(CONFIG.programId || "default", "magenta")}`,
    );
  ...
  async testBasicPagination() {
    ...
    for (const limit of limits) {
      ...
      console.log(
        `   Trades found: ${colorize(result.trades.length.toString(), result.trades.length > 0 ? "green" : "red")}`,
      );
      console.log(
        `   Has more: ${colorize(result.hasMore.toString(), result.hasMore ? "yellow" : "green")}`,
      );
      ...
      if (result.hasMore && !result.lastSignature) {
        console.log(
          colorize(
            "   ⚠️  WARNING: hasMore=true but lastSignature is undefined",
            "red",
          ),
        );
      }
  ...
  async testFullPaginationFetch() {
    ...
    while (hasMore && pageCount < MAX_PAGES) {
      ...
      console.log(
        `   Page ${pageCount}: ${colorize(result.trades.length.toString(), "green")} trades`,
      );
      console.log(
        `   Total so far: ${colorize(allTrades.length.toString(), "cyan")}`,
      );
      console.log(
        `   Has more: ${colorize(hasMore.toString(), hasMore ? "yellow" : "green")}`,
      );
      ...
      if (duplicates > 0) {
        console.log(
          colorize(
            `   ⚠️  WARNING: Found ${duplicates} duplicate transactions`,
            "red",
          ),
        );
      }
    ...
    console.log(
      colorize(
        `\n✅ Fetched ${allTrades.length} trades across ${pageCount} pages in ${duration}s`,
        "green",
      ),
    );
  ...
  async testIncrementalFetch() {
    ...
    if (initialResult.trades.length === 0) {
      console.log(
        colorize(
          "⚠️  No initial trades found, skipping incremental test",
          "yellow",
        ),
      );
    ...
    console.log(
      `   Most recent signature: ${colorize(mostRecentSignature.slice(0, 16) + "...", "dim")}`,
    );

    // Simulate incremental fetch - get transactions until we hit the most recent
    console.log(
      colorize(
        "\n🔄 Simulating incremental fetch (until = most recent)...",
        "yellow",
      ),
    );
    ...
    console.log(
      `   Found ${colorize(incrementalResult.trades.length.toString(), "green")} trades newer than most recent?`,
    );
    console.log(
      `   This should be 0 (or very few depending on timing): ${incrementalResult.trades.length}`,
    );
    ...
    if (hasOlderTrades) {
      console.log(
        colorize(
          "   ⚠️  WARNING: Found trades older than until signature",
          "red",
        ),
      );
    }
  ...
  async testBetweenSignatures() {
    ...
    if (referenceResult.trades.length < 2) {
      console.log(
        colorize("⚠️  Not enough trades to test between signatures", "yellow"),
      );
    ...
    const untilSig =
      referenceResult.trades[referenceResult.trades.length - 1].transactionHash;

    console.log(
      `   Before: ${colorize(beforeSig.slice(0, 16) + "...", "dim")}`,
    );
    ...
    console.log(
      `   Found ${colorize(betweenResult.trades.length.toString(), "green")} trades between signatures`,
    );
    console.log(
      `   Expected range: 0-${referenceResult.trades.length - 1} trades`,
    );
    ...
    betweenResult.trades.forEach((trade) => {
      if (
        trade.transactionHash === beforeSig ||
        trade.transactionHash === untilSig
      ) {
        outOfBounds++;
      }
    });
    ...
    if (outOfBounds > 0) {
      console.log(
        colorize(
          `   ⚠️  WARNING: Found ${outOfBounds} trades outside the specified range`,
          "red",
        ),
      );
    }
  ...
  async testPerformanceComparison() {
    ...
    console.log(
      `   Duration: ${colorize(allDuration.toFixed(2) + "s", allDuration < 2 ? "green" : "red")}`,
    );
    ...
    console.log(
      `   Duration: ${colorize(paginatedDuration.toFixed(2) + "s", paginatedDuration < 1 ? "green" : "red")}`,
    );
    ...
    console.log(
      `   Duration: ${colorize(multiDuration.toFixed(2) + "s", multiDuration < 3 ? "green" : "red")}`,
    );
  ...
  async testEdgeCases() {
    ...
    try {
      ...
      console.log(
        `   ✅ Passed: ${result.trades.length === 0 ? "Empty array" : "Has trades"}`,
      );
    ...
    try {
      ...
      console.log(
        `   ✅ Passed: ${result.trades.length === 0 ? "Empty array" : "Has trades"} (should be empty)`,
      );
  ...
  private validateTradeStructure(trade: TradeRecord, context: string) {
    ...
    if (missingFields.length > 0) {
      console.log(
        colorize(
          `   ⚠️  Trade missing fields in ${context}: ${missingFields.join(", ")}`,
          "red",
        ),
      );
    }
    ...
    if (trade.fees) {
      ...
      const missingFeeFields = feeFields.filter(
        (field) => !(field in trade.fees!),
      );
      if (missingFeeFields.length > 0) {
        console.log(
          colorize(
            `   ⚠️  Fee missing fields: ${missingFeeFields.join(", ")}`,
            "red",
          ),
        );
      }
    ...
    if (trade.logType?.includes("Fill") && !trade.discriminator) {
      console.log(
        colorize(`   ⚠️  Fill event missing discriminator`, "yellow"),
      );
    }
  }
  ...
  async generateReport() {
    ...
    for (const [testName, results] of this.results) {
      ...
      if (testName === "basicPagination" && results.results) {
        results.results.forEach((r: any) => {
          console.log(
            `  - Limit ${r.limit}: ${r.trades} trades, hasMore: ${r.hasMore}`,
          );
        });
        ...
        console.log(
          `  fetchAll(): ${results.allTrades} trades in ${results.allDuration.toFixed(2)}s`,
        );
        console.log(
          `  paginated(): ${results.paginatedTrades} trades in ${results.paginatedDuration.toFixed(2)}s`,
        );
        console.log(
          `  multi-page: ${results.multiPageTrades} trades in ${results.multiPageDuration.toFixed(2)}s`,
        );
    ...
    console.log(
      colorize(`⏱️  Total Test Duration: ${totalDuration.toFixed(2)}s`, "cyan"),
    );
  ...
  async exportResults() {
    ...
    const reportPath = path.join(
      CONFIG.exportDir,
      `test-report-${walletShort}-${timestamp}.json`,
    );
...
async function runAllTests() {
  console.log(
    colorize(
      "\n🚀 TransactionDataFetcher Comprehensive Test Suite",
      "bright",
      "cyan",
    ),
  );
...
const commands: Record<string, () => Promise<any>> = {
  basic: async () =>
    (await new TestHarness().initialize())
      .testBasicPagination()
      .then((h) => h.generateReport()),
  full: async () =>
    (await new TestHarness().initialize())
      .testFullPaginationFetch()
      .then((h) => h.generateReport()),
  incremental: async () =>
    (await new TestHarness().initialize())
      .testIncrementalFetch()
      .then((h) => h.generateReport()),
  between: async () =>
    (await new TestHarness().initialize())
      .testBetweenSignatures()
      .then((h) => h.generateReport()),
  performance: async () =>
    (await new TestHarness().initialize())
